...
IOReturn RealtekR1000::selectMedium(const IONetworkMedium *medium)
{
	...
	{	
		this->setLinkStatus(kIONetworkLinkValid, NULL, 0, NULL);
	}
...
bool RealtekR1000::AllocateBufferMemory()
{
	...
	{
		DLog("Failed to allocate %lu bytes for Rx buffer pool\n", rxBufferPoolSize);
		return false;
	}
	...
	{
		DLog("Failed to allocate %lu bytes for Tx packet buffers\n", rxBufferPoolSize);
		return false;
	}
